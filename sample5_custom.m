...
%Settings:
coll.NumMarkers      =1;          %Number of markers in the collection.         
...
coll.CollectionTime  =2.0;          %Number of seconds of data to collect.        
coll.PreTriggerTime  =0;          %Number of seconds to pre-trigger data by.    
coll.Flags={'OPTOTRAK_NO_FIRE_MARKERS_FLAG';'OPTOTRAK_BUFFER_RAW_FLAG';'OPTOTRAK_GET_NEXT_FRAME_FLAG'};
...
SpoolComplete=0;
[fnumber, t] = deal(nan(coll.CollectionTime*coll.FrameFrequency+5,1));
tic;
i = 0;
while(~SpoolComplete)
...
%     end
    %After having received 3D, request a new frame of realtime 3D data:
    optotrak('RequestLatest3D')
  end
end
% fprintf('Spool Status: 0x%04x\n',SpoolStatus);

%Eat away requested but not yet received 3D data (otherwise device is locked!)
while(optotrak('DataIsReady'))
  optotrak('DataReceiveLatest3D',coll.NumMarkers);
end
