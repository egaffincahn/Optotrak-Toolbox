...
%% load
addpath('C:\Users\landyadmin\Documents\Optotrak')
...
% [calibration, opt] = OptoCalibEnv();
% [calibration, opt] = OptoCalibFinger(opt, calibration);
...
while ~KbCheck(-1)
    ...
    count = -1;
    xy = nan(2,size(combinations,1)*2*3);
    for i = 1:min([size(combinations,1),size(ptb_wingcalc,1)])
        for j = 1:3
            count = count + 2;
            xy(:,count:count+1) = [...
                ptb_all(combinations(i,j),1) ptb_wingcalc(i,1);...
                ptb_all(combinations(i,j),2) ptb_wingcalc(i,2)...
                ];
        end
    end
    Screen('DrawLines', w, xy, [], [75 75 75]);
    Screen('FillOval', w, [0 180 180], pt2oval(ptb_wingcalc, 3)); % wing calc
    Screen('FillOval', w, [128 128 0], pt2oval(ptb_all,3)); % all 6
    Screen('FillRect', w, [0 0 128], pt2oval(nanmedian(ptb_wingcalc,1), 8));
    %Screen('FrameRect', w, [255 255 255], pt2oval(nanmedian(ptb_wingcalc,1), 8));
    Screen('FillOval', w, [0 0 128], pt2oval(nanmean(ptb_wingcalc,1), 8));
    %Screen('FrameOval', w, [255 255 255], pt2oval(nanmean(ptb_wingcalc,1), 8));
    Screen('Flip', w);
end
catch me
    OptoFail
