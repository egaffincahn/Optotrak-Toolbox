...
%     [calibration, collection_params] = OptoCalibLoad;
    ...
    while go
        ...
        missing = true;
        while missing
            [~, ~, missing, ~, ~, init, ~] = OptoCollect(collection_params, calibration);
            WaitSecs(.05);
        end
        ...
        while true
            ...
            [~, x_ptb, ~, ~, ~, xyz, ~] = OptoCollect(collection_params, calibration);
            vdist = max([xyz(3) - init(3), 0]); % get distance above table
            circ_rects = [circ_rects, circ2ptb(x_ptb, dist2rad(vdist))]; %#ok<*AGROW>
